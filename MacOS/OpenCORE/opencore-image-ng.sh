...
BASE="$(dirname $0)"
cp -a $BASE/EFI $WORK
cp -a $BASE/startup.nsh $WORK
cp -a $BASE/${ChipSet} $WORK
find "$WORK"
...
cp -v "$cfg" $WORK/${ChipSet}/config.plist
fish mkdir /ESP/EFI
...
# Note
fish copy-in $WORK/startup.nsh /

BASE="$(dirname $0)"
fish copy-in "$WORK/${ChipSet}/config.plist"               /ESP/EFI/OC/
