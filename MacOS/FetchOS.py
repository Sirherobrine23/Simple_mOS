...
def save_image(url, sess, filename='', directory=''):
    ...
    with open(os.path.join(directory, filename), 'wb') as fhandle:
        ...
        while True:
            ...
            fhandle.write(chunk)
            size += len(chunk)
            # print('{} MBs downloaded...'.format(size / (2 ** 20)), end='')
            sys.stdout.flush()
    ...
    for model in db:
        try:
            if anon:
                ...
                model_latest = get_image_info(session, bid=model, mlb=MLB_ZERO, diag=False, os_type='default')
...
def main():
    ...
    parser.add_argument('-sl', '--select', type=int, default=3,
                        help='Select Os')
    ...
    products = json.load(open(os.path.join(SELF_DIR, 'products.json'),))
    ...
    print('MacOS Download Version: ' + product["name"])
